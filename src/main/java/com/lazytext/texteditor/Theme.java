...
public class Theme {
	...
	public static void themeTextAreaSelectedTextColor(Color bgColor, Color fgColor) {
	...
	public static void themeTextArea(JTextArea textArea, Color fgColor, Color bgColor) {
	...
	public static void themeVerticalScrollBar(JScrollPane scrollPane, final Color scrollBarThumbColor) {
	...
	public static void themeHorizontalScrollBar(JScrollPane scrollPane, final Color scrollBarThumbColor) {
	...
	public static void themeScrollBar(JScrollPane scrollPane, Color scrollBarThumbColor, Color bgColor) {
	...
	public static void themeStatusBarComponents(JToolBar statusBar, Color fgColor, Color bgColor) {
	...
	public static void themeStatusBar(JToolBar statusBar, Color fgColor, Color bgColor) {
		statusBar.setBackground(bgColor);
		themeStatusBarComponents(statusBar, fgColor, bgColor);
	} // End of themeStatusBar

	public static void themeMenuBarItems(JMenuBar menuBar, Color fgColor, Color bgColor) {
	...
	public static void themeMenuBar(JMenuBar menuBar, Color fgColor, Color bgColor) {
	...
	public static void changeStatusBarFont(JToolBar statusBar, Font statusBarFont) {
		int numberOfComponents = statusBar.getComponentCount();
		for (int i = 0; i < numberOfComponents; i++) {
	...
	public void changeFont(Font menuBarFont, Font textAreaFont, Font statusBarFont) {
	...
	private void assignValuesToMap(Node currentNode) {
		if(currentNode.getNodeName().equals("components")) {
			customComponentValues.put(currentNode.getTextContent(), getAllAttributes(currentNode));
		} else {
	...
	public void getThemeValues(String themeFile) throws IOException { this.parseThemeValues(themeFile); }
	...
	public void applyFontValues() {
		...
		if(!checkValues) {
			windowFonts[0] = new Font(fontValues[0], Font.PLAIN, Integer.parseInt(fontValues[1]) - 11);
			windowFonts[1] = new Font(fontValues[0], Font.PLAIN, Integer.parseInt(fontValues[1]));
			windowFonts[2] = new Font(fontValues[0], Font.PLAIN, Integer.parseInt(fontValues[1]) - 11);
		} else {
	...
	public void applyMenuValues() {
		themeMenuBar(this.window.getMenuBar(), getColorValue(themeValues.get("menuBar")[1]), getColorValue(themeValues.get("menuBar")[0]));
	} // End of applyMenuValues

	public void applyTextAreaValues() {
		
		themeScrollBar(this.window.getScrollPane(), getColorValue(themeValues.get("textAreaScrollBars")[1]), getColorValue(themeValues.get("textAreaScrollBars")[0]));

		themeTextArea(this.window.getLinedNumberArea(), getColorValue(themeValues.get("lineNumberArea")[1]), getColorValue(themeValues.get("lineNumberArea")[0]));

		themeTextArea(this.window.getTextArea(), getColorValue(themeValues.get("textArea")[2]), getColorValue(themeValues.get("textArea")[0]));

		themeTextAreaCaret(this.window.getTextArea(), getColorValue(themeValues.get("textArea")[1]));
	} // End of applyTextAreaValues

	public void applyStatusBarValues() {
		themeStatusBar(this.window.getStatusBar(), getColorValue(themeValues.get("statusBar")[1]), getColorValue(themeValues.get("statusBar")[0]));
	} // End of applyStatusBarValues

	private void writeLastThemeUsed(String themeName) throws IOException {
		PrintWriter writer = new PrintWriter(this.themeDirectory + "last_used_theme.txt");
		writer.print("");
