...
public class NewLineListener implements DocumentListener {
  ...
  private String getLineNumber() {
    ...
    // Will always loop up to the current number of lines in the text area + 2
    // such that if number of lines is 3 root.getElementIndex(caretPosition) + 2
    // = 5 since elementIndex always starts from the second line in order to
    // account for when there is only one line displayed this will always be 2
    // lines ahead from previous number of lines in order to add a new line such
  ...
  private void checkIfNewLineWasInserted() {
    ...
    // Only adds line number when new line is added and not when typing on a
    // line
